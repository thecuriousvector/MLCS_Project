...
def get_start(path):
	...
	print path+"oyez_full/fullTierName.csv"# change to the proper direction
	whole_list = pd.read_csv(path+"oyez_full/fullTierName.csv")#contain infomation about all audio text transcription file
...
def get_audio_feature(path,year,filename,whole_list):
	...
	# define new column
		...
		if not os.path.isfile('audio_feature_1.csv'):
			df.to_csv('audio_feature_1.csv',header ='column_names',index = False)
		else:
			df.to_csv('audio_feature_1.csv',mode = 'a',header=False,index = False)
