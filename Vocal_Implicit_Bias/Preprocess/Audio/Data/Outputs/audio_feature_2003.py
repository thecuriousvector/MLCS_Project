...
def get_start(path):
	...
	print (path+"oyez_full/fullTierName_2003.csv")# change to the proper direction
	whole_list = pd.read_csv(path+"oyez_full/fullTierName_2003.csv")#contain infomation about all audio text transcription file
...
def get_audio_feature(path,year,filename,whole_list):
	...
	filepath = path+"FAVE/FAVE-extract/"+"2003"+"_vowels/ABA/"+str(filename)+".csv"
	#print path+"FAVE/FAVE-extract/"+"2003"+"_vowels/ABA/"+str(filename)+".csv"
	if os.path.isfile(filepath)==False:
		return
	else:
		print (path+"FAVE/FAVE-extract/"+"2003"+"_vowels/ABA/"+str(filename)+".csv")
	feature_list = pd.read_csv(path+"FAVE/FAVE-extract/"+"2003"+"_vowels/ABA/"+str(filename)+".csv")
	...
	# define new column
		...
		# save or add these column into the audio_feature_2003.csv file
		if not os.path.isfile('audio_feature_2003.csv'):
			df.to_csv('audio_feature_2003.csv',header ='column_names',index = False)
		else:
			df.to_csv('audio_feature_2003.csv',mode = 'a',header=False,index = False)
