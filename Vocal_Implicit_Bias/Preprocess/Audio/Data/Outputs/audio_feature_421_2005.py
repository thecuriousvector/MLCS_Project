...
def get_start(path):
	...
	print path+"oyez_full/fullTierName_2005.csv"
	whole_list = pd.read_csv(path+"oyez_full/fullTierName_2005.csv")
	...
	#log file
	if os.path.isfile('log_2005.bin'):
		with open("log_2005.bin","r") as log_file:
	...
	with open("log_2005.bin","a+") as log_file: 
...
def get_audio_feature(path,year,filename,whole_list):
	...
	filepath = path+"FAVE/FAVE-extract/"+"2005"+"_vowels/AXBYA/"+str(filename)+".csv"

	#check exist
	if os.path.isfile(filepath)==False:
		return
	else:
		print path+"FAVE/FAVE-extract/"+"2005"+"_vowels/AXBYA/"+str(filename)+".csv"

	feature_list = pd.read_csv(path+"FAVE/FAVE-extract/"+"2005"+"_vowels/AXBYA/"+str(filename)+".csv")
	...
	if df.shape[0]!=0:
		...
		if not os.path.isfile('audio_feature_axbya_2005.csv'):
			df.to_csv('audio_feature_axbya_2005.csv',header ='column_names',index = False)
		else:
			df.to_csv('audio_feature_axbya_2005.csv',mode = 'a',header=False,index = False)
